...
import { Package, MapPin, Clock, User, Phone, CheckCircle, X } from "lucide-react"
...
export default function OrdersPage() {
  ...
  const handleAcceptOrder = async (orderId) => {
    ...
    if (orderToAccept) {
      const response = await axios.patch(`${process.env.REACT_APP_BASE_URL}/api/agent/${orderId}/accept`);
  ...
  const handleMarkAsDelivered = async (orderId) => {
    ...
    if (orderToDeliver && deliveryOtp) {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/agent/${orderId}/deliver`, { deliveryOtp });
      if (response.data.success) {
        setRefetch(1);
  ...
  const handleRequestCancellation = async (orderId) => {
    ...
    const orderToDeliver = acceptedOrders.find((order) => order._id === orderId)
    if (orderToDeliver) {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/agent/${orderId}/request-cancel`, { message: cancellationReason });
      if (response.data.success) {
        setRefetch(1);
  ...
  useEffect(() => {
    const getAvailableOrders = async () => {
      try {
        const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/orders/available-orders/`)
        if (response.data.success) {
  ...
  useEffect(() => {
    const getAvailableOrders = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/agent/my-orders/`)
        if (response.data.success) {
