  ...
  persist(
    (set, get) => ({
      ...
      login: async (email, password, phone) => {
        try {
          set({ loading: true });
          // console.log("object", process.env.REACT_APP_BASE_URL);
      ...
      agentRegister: async (userData) => {
        try {
          const response = await axios.post(
            `${process.env.REACT_APP_BASE_URL}/api/auth/agent/register`,
  ...
  persist(
    (set, get) => ({
      ...
      hydrateCart: async (location) => {
        ...
        const { data: products } = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/products/bulk`, {
